...
function julia_wrapper_atria(ARGS::Vector{String}; exit_after_help = true)
    ...
    AdapterTrim = do_adapter_trimming ? quote
        ...
        extra_tolerance = max(r1_adapter_nmatch, r2_adapter_nmatch) < $trim_score
        ...
        if max_nmatch > $trim_score
            ...
            if r1_adapter_nmatch < $trim_score
                # r1_adapter_pos, r1_adapter_nmatch = bitwise_scan($adapter1_seqheadset, r1.seq, max_adapter_pos, $(kmer_tolerance+3), until=max_adapter_pos+1)
                ...
                if r1_adapter_nmatch2 > $trim_score
                    r1_adapter_pos = r1_adapter_pos2
                    r1_adapter_nmatch = r1_adapter_nmatch2
                end
            end
            if r2_adapter_nmatch < $trim_score
                # r2_adapter_pos, r2_adapter_nmatch = bitwise_scan($adapter2_seqheadset, r2.seq, max_adapter_pos, $(kmer_tolerance+3), until=max_adapter_pos+1)
                ...
                if r2_adapter_nmatch2 > $trim_score
                    r2_adapter_pos = r2_adapter_pos2
                    r2_adapter_nmatch = r2_adapter_nmatch2
                end
            end
            if r1_pe_nmatch < $trim_score
                # r1_rc_pos = length(r1.seq) - max_adapter_pos + 2
                # r1_insert_size_pe, r1_pe_nmatch = bitwise_scan(r2_seqheadset, r1_seq_rc, r1_rc_pos, $(kmer_tolerance+3), until=r1_rc_pos+1)
                # r1_insert_size_pe = r1_insert_size_pe == 0 ? 0 : length(r1.seq) - r1_insert_size_pe + 1
                ...
                if r1_pe_nmatch2 > $trim_score
                    r1_insert_size_pe = r1_insert_size_pe2 == 0 ? 0 : length(r1.seq) - r1_insert_size_pe2 + 1
                    r1_pe_nmatch = r1_pe_nmatch2
                end
            end
            if r2_pe_nmatch < $trim_score
                # r2_rc_pos = length(r2.seq) - max_adapter_pos + 2
                # r2_insert_size_pe, r2_pe_nmatch = bitwise_scan(r1_seqheadset, r2_seq_rc, r2_rc_pos, $(kmer_tolerance+3), until=r2_rc_pos+1)
                # r2_insert_size_pe = r2_insert_size_pe == 0 ? 0 : length(r2.seq) - r2_insert_size_pe + 1
        ...
        r1_insert_size_decision, r2_insert_size_decision, r12_score = insert_size_decision_separate(r1_insert_size_real, r1_score, r2_insert_size_real, r2_score, insert_size_diff=$r1_r2_diff, score_diff=$score_diff)

        # stats in FqRecord:
        @static if $do_read_stats
            adapter_trimming_stat = @sprintf "\tdecision:\t%d\t%d\t%.2f\tr1:\t%d\t%.2f\t%d\t%.2f\tr2:\t%d\t%.2f\t%d\t%.2f" r1_insert_size_decision r2_insert_size_decision r12_score r1_insert_size r1_adapter_score r1_insert_size_pe r1_pe_score r2_insert_size r2_adapter_score r2_insert_size_pe r2_pe_score
    ...
    for filenum in file_range
        ...
        isingzip = occursin(r"\.gz$", file1)
        outgzip = uppercase(args["gzip"])
        isoutgzip = outgzip == "AUTO" ? isingzip :
                    outgzip == "YES"  ? true   : false
        ...
        if isingzip
            io1 = open(`pigz -cd $file1`, write=false)
            io2 = open(`pigz -cd $file2`, write=false)
        else
            io1 = open(file1, "r")
            io2 = open(file2, "r")
        end
        ...
        function cycle_wrapper()
            ...
            if typeof(io1) <: IOStream  # not compressed
                ...
                @timeit to "Read" (n_r1, n_r2, r1s, r2s, ncopied) = load_fqs_threads!(io1, io2, in1bytes, in2bytes, vr1s, vr2s, r1s, r2s; remove_first_n = n_reads, njobs=njobs)
            else  # gziped
                ...
                @timeit to "Read" (n_r1, n_r2, r1s, r2s, in1bytes_nremain, in2bytes_nremain, ncopied) = load_fqs_threads!(
                    io1, io2,
            ...
            @timeit to "Process" processing_reads_threads!(r1s, r2s, isgoods, n_reads)
            ...
            @timeit to "Write" write_fqs_threads!(
                io1out::IO, io2out::IO,
        ...
        @timeit to "Compiling" if !eof(io1::IO) || !eof(io2::IO)
            cycle_wrapper()
        end

        @timeit to "Compiled" while !eof(io1::IO) || !eof(io2::IO)
            cycle_wrapper()
        end
